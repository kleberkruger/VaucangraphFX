...
class ResourceBundleInjector {
     ...
     * @param resourceBundle the ResourceBundle instance that is used.
     */
    static void injectResourceBundle(Object target, ResourceBundle resourceBundle) {
        ...
        if (notAssignableFieldPresent) {
            throw new IllegalStateException(
                    "The class [" + target + "] has at least one field with the annotation " +
                            "@InjectResourceBundle but the field is not of type ResourceBundle.");
        }
        ...
        if (resourceBundle == null || resourceBundle.equals(EMPTY_RESOURCE_BUNDLE)) {
            ...
            if (!fieldsWithAnnotation.isEmpty()) {
                ...
                if (nonOptionalFieldsPresent) {
                    throw new IllegalStateException(
                            "The class [" + target + "] expects a ResourceBundle to be injected but no ResourceBundle was defined while loading.");
                }
            }
        } else {
            fieldsWithAnnotation
                    .forEach(field -> {
                        if (field.getType().isAssignableFrom(ResourceBundle.class)) {
                            ...
                            throw new IllegalStateException("The class [" + target + "] has a field with the " +
                                    "@InjectResourceBundle annotation but the type of the field doesn't match ResourceBundle");
                        }
                    });
        }
    }
