...
public class FxmlViewLoader {
     ...
     *
     * @param viewType         the type of the view to be loaded.
     * @param resourceBundle   the resourceBundle that is passed to the
     *                         {@link FXMLLoader}.
     * @param codeBehind       the controller instance that is passed to the
     *                         {@link FXMLLoader}
     * @param root             the root object that is passed to the
     *                         {@link FXMLLoader}
     * @param viewModel        the viewModel instance that is used when loading the
     *                         viewTuple.
     ...
     *
     * @param resource         the string path to the fxml file that is loaded.
     * @param resourceBundle   the resourceBundle that is passed to the
     *                         {@link FXMLLoader}.
     * @param codeBehind       the controller instance that is passed to the
     *                         {@link FXMLLoader}
     ...
     */
    public <ViewType extends View<? extends ViewModelType>, ViewModelType extends ViewModel> ViewTuple<ViewType, ViewModelType> loadFxmlViewTuple(
            final Class<? extends ViewType> viewType, final String resource, ResourceBundle resourceBundle, final ViewType codeBehind, final Object root,
            ViewModelType viewModel, Context parentContext, Collection<Scope> providedScopes, List<BuilderFactory> builderFactories) {
        ...
        try {
            ...
            final FXMLLoader loader = createFxmlLoader(viewType, resource, resourceBundle, codeBehind, root, viewModel, context, viewInSceneProperty, builderFactories);
    ...
    /**
     * Método que cria um FXMLLoader capaz de ler arquivos FXML em quaisquer módulos do programa, desde que o módulo
     * esteja aberto (open) ou que o pacote do recurso FXML esteja aberto (opens).
     * <p>
     * ================================================================================
     * Modificação deste método:
     * --------------------------------------------------------------------------------
     * no lugar de:
     * final URL location = FxmlViewLoader.class.getResource(resource);
     * <p>
     * foi sobrescrito para:
     * final URL location = viewType.getResource(resource);
     * --------------------------------------------------------------------------------
     *
     * @param viewType            (ADICIONADO) - Classe CodeBehind usada para carregar o recurso
     * @param resource
     ...
     * @param viewModel
     * @param context
     * @param viewInSceneProperty
     * @param builderFactories
     * @return
     * @throws IOException
     */
    private FXMLLoader createFxmlLoader(Class<? extends View> viewType, String resource, ResourceBundle resourceBundle, View codeBehind, Object root,
...
//        System.out.println("Resource: " + viewType.getResource(resource));

        // Load FXML file
        final URL location = viewType.getResource(resource);
        if (location == null) {
            throw new IOException("Error loading FXML - can't load from given resource path: " + resource);
        }
        ...
        if (viewModel != null && codeBehind == null) {
            fxmlLoader.setControllerFactory(new ControllerFactoryForCustomViewModel(viewModel, resourceBundle, context, viewInSceneProperty));
        } else {
