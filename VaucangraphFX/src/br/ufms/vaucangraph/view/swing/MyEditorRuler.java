 ...
 */
public final class MyEditorRuler extends JComponent implements MouseMotionListener, DropTargetListener {
    ...
    /**
     * Specified that start and length of the active region, ie the region to paint with the background border. This is
     * used for example to indicate the printable region of a graph.
     */
    protected int activeoffset, activelength;

    /**
     * Specifies the scale for the metrics. Default is {@link JGraphEditorDiagramPane#DEFAULT_PAGESCALE}.
     */
    protected double scale = DEFAULT_PAGESCALE;

    /**
     * Specifies the unit system. Default is {@link JGraphEditorDiagramPane#DEFAULT_ISMETRIC}.
     */
     ...
     *
     */
    protected transient mxIEventListener repaintHandler = (Object source, mxEventObject evt) -> {
        repaint();
    };
     ...
     */
    public MyEditorRuler(mxGraphComponent graphComponent, int orientation) {
        ...
        graphComponent.getGraphControl().addMouseMotionListener((MouseMotionListener) this);

        DropTarget dropTarget = graphComponent.getDropTarget();

        try {
            if (dropTarget != null) {
                dropTarget.addDropTargetListener((DropTargetListener) this);
            }
    ...
    /**
     * Returns the preferred size by replacing the respective component of the graph's preferred size with
     * {@link #rulerSize}.
     *
    ...
    public void paintComponent(Graphics g) {
        ...
        double sc = graph.getView().getScale(); // sc: scale (local)
        double tx = trans.getX() * sc;
        double ty = trans.getY() * sc;
